...
$allowed_filters = ['all', 'projects', 'users', 'hobbies_skills'];
if (!in_array($filter, $allowed_filters, true)) {
    $filter = 'all';
}

$show_projects = ($filter === 'all' || $filter === 'projects');
$show_users    = ($filter === 'all' || $filter === 'users' || $filter === 'hobbies_skills');
$tag_skill_only = ($filter === 'hobbies_skills');
    ...
    if ($show_users) {
        ...
        if ($tag_skill_only) {
            $user_sql = "
                ...
                WHERE
                    ut.tag_name LIKE CONCAT('%', ?, '%')
                    OR s.skill_name LIKE CONCAT('%', ?, '%')
                GROUP BY u.user_id
...
<div class="col-lg-10 content-area">
    ...
    <form method="GET" action="search.php" class="search-filter-form" style="margin-bottom: 20px;">
        ...
        <label style="margin-right: 15px;">
            <input type="radio" name="filter" value="hobbies_skills" <?php echo $filter === 'hobbies_skills' ? 'checked' : ''; ?>>
                ...
                <?php foreach ($users as $u): ?>
                    <div class="project-card">
                        <div class="project-card-header">
                            ...
                            <span>
                                <?php echo htmlspecialchars($u['course'] ?? '—'); ?> - Year <?php echo (int)($u['year'] ?? 0); ?>
                                <?php if (($u['status'] ?? 'active') === 'suspended'): ?>
                                    <span style="margin-left: 10px; color: #c0392b; font-weight: 700;">Suspended</span>
                                <?php endif; ?>
                            </span>
                        </div>
                        ...
                        <div class="project-card-body">
                            ...
                            <p><strong>Skills:</strong> <?php echo htmlspecialchars($u['skills'] ?? 'None'); ?></p>
                            <p><strong>Interests &amp; Hobbies:</strong> <?php echo htmlspecialchars($u['tags'] ?? 'None'); ?></p>
                        </div>

                        <div class="project-card-actions">
                            <?php if (($u['status'] ?? 'active') === 'suspended'): ?>
                                <button type="button" class="details-btn" disabled>Profile Unavailable</button>
                            <?php else: ?>
